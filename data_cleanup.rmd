...
# filtering 
    ...
    english_country_current <- english_by_country %>%
      filter(country == "India")
    
    english_country_current_summary <- english_=country_current %>%
...
ggplot(immigration_country_current, 
       aes(x = year, y = count, fill = admission_class)) +
      geom_area(position = 'stack') + 
      labs(y = "Total Immigrants", x = "Year", fill = "Admission Class") + 
      scale_fill_discrete(labels = c("Diversity", "Employment","Immediate Relatives", "Other Relatives", "Refugee", "Other")) +  
      scale_x_continuous(breaks = c(2009, 2011, 2013, 2015, 2017))

ggplot(english_country_current, aes(x = year, y = percentage_very_well)) +
  geom_line() + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1.1)) + 
  labs(y = "% Speaking English Very Well", x = "Year")

ggplot(english_country_current_summary, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=english)) +
     geom_rect() +
     coord_polar(theta="y") +
     xlim(c(2, 4)) + 
     scale_fill_discrete(labels = c("Not Very Well", "Very Well")) +
     labs(title = paste0(percent(english_country_current_summary$fraction[1]) ,"  people in ", " Speak English Very Well"), fill = "English Speaking Ability") +
...
immigration_country_top_10 <- immigration_country %>%
                                    filter(country %in% ranking$country)




ranking_20 <- immigration_country %>%
              filter(year == 2009) %>%
              arrange(desc(total)) %>%
              select(country) %>%
              head(20)
...
immigration_country_top_20 <- immigration_country %>%
                                    filter(country %in% ranking_20$country, year == 2009) %>%
                                    arrange(desc(total))



ggplot(immigration_country_top_20, aes(x = reorder(country, -total), y = total)) + geom_col(aes(fill = total)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + scale_fill_gradient2(low = "blue", high = "red", midpoint = median(immigration_country_top_20$total)) + labs(x = "Country", y = "Total Immigrants","Top ")
...
ggplot(immigration_country_top_10, aes(x = `total`, y = `country`, fill = ..x..)) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  scale_fill_viridis(name = "total", option = "C") +
